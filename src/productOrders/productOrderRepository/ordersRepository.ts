...
export class OrderRepository extends Repository<OrderEntity> {
    ...
    createOrder = async (user: AuthEntity, orderDto: OrderDto): Promise<OrderObject> => {
        const {
            orderName, 
            imageUrl, 
            quantity, 
        ...
        try {
            await newOrder.save()
            this.logger.verbose(`order with id ${newOrder.orderId} saved successfully`)
            return ({
    ...
    fetchOrderWithId = async (user: AuthEntity, orderId: string): Promise<OrderEntity> => {
