...
export class CustomPackageOrderRepository extends Repository<CustomPackageOrderEntity> {
  ...
  async getCustomPackageOrder(user: AuthEntity): Promise<CustomPackageOrderEntity[]> {
    const query = this.createQueryBuilder('orderName');
    query.where('orderName.userId = :userId', {userId: user.id});
    
    const orders = await query.getMany()
    try {
        this.logger.verbose(`custom orders by user ${user.id} successfully fetched`)
        return orders;
    } catch (error) {
  ...
  async getCustomPackageOrderWithId(customPackageId: string, user: AuthEntity): Promise<CustomPackageOrderEntity> {
    ...
    try{
        ...
        this.logger.error(`order with id ${customPackageId} unsuccessfully fetched`);
        throw new InternalServerErrorException(`failed to fetch id ${customPackageId}`)
    }
  }
  ...
  async updateCustomPackageOrder(
    ...
    const {orderName, item, deliveryDate, addInfo} = updateCustomPackageOrderDto;
    ...
    try {
        ...
        this.logger.verbose(`user ${user.firstname} has successfully updated order with id ${customPackageId}`)
        return order;
    } catch (error) {
        this.logger.error(`failed to update order with id ${customPackageId}`)
        throw new InternalServerErrorException(`failed to update order with id ${customPackageId}`)
    }
  }
} 